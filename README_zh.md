...
以下所有操作都可以在浏览器中完成，无需安装任何软件。

---

## 快速开始 — 你想做什么？

> **白币 (WC)** 是链的原生货币，通过运行验证节点获得。**蓝币** 是任何组织发行的代币，通过内置 AMM 买卖。

### 「我想创建一个钱包」

1. 打开 [http://8.217.52.231](http://8.217.52.231) → 点击 **Wallet**
2. 设置密码 → 点击 **Create Wallet**
3. **抄下 12 个助记词**（这是你的备份！）
4. 勾选 "I have saved my mnemonic" → 下载 keystore 文件
5. 完成，地址显示在页面上

### 「我想获取一些白币」

1. 打开 [http://8.217.52.231](http://8.217.52.231) → 点击 **Faucet**
2. 粘贴你的地址 → 点击 **Claim**
3. 你会收到 100 WC（每个地址每 24 小时可领取一次）

### 「我想给别人转账」

1. 进入 **Wallet** → 用 keystore 文件 + 密码解锁
2. 选择 **Transfer WC** 标签页 → 输入收款地址和金额 → **Send**

### 「我想发行自己的蓝币」

1. 进入 **Wallet** → 解锁
2. 选择 **Deploy** 标签页
3. 填写：名称、符号、池比例、团队比例、初始白币、每月释放量
4. 点击 **Deploy** → 你的代币和 AMM 池即刻上线

### 「我想交易蓝币」

1. 进入 **Wallet** → 解锁
2. 选择 **Swap** 标签页
3. 选择代币，选择方向（白币→蓝币 或 蓝币→白币），输入金额 → **Swap**
...
节点会自动生成钱包、连接测试网、开始 24 小时候选期。通过验证后，每出一个块（每 15 秒）获得 50 WC。

### 「我只想看看链上数据」

访问 [http://8.217.52.231](http://8.217.52.231) — 浏览区块、交易、验证者、蓝币和任意地址。
