...
以下のすべての操作はブラウザから実行できます。インストール不要です。

---

## クイックスタート — 何をしたいですか？

> **White Coin (WC)** はチェーンのネイティブ通貨で、バリデータノードを運用して獲得します。**Blue Coin** は任意の組織が発行するトークンで、組み込みのAMMで売買できます。

### 「ウォレットを作りたい」

1. [http://8.217.52.231](http://8.217.52.231) にアクセス → **Wallet** をクリック
2. パスワードを設定 → **Create Wallet** をクリック
3. **12個のニーモニックフレーズを書き留める**（これがバックアップです！）
4. "I have saved my mnemonic" にチェック → keystoreファイルをダウンロード
5. 完了。アドレスが画面に表示されます

### 「White Coinを手に入れたい」

1. [http://8.217.52.231](http://8.217.52.231) にアクセス → **Faucet** をクリック
2. アドレスを貼り付け → **Claim** をクリック
3. 100 WCを受け取れます（1アドレスにつき24時間に1回）

### 「トークンを送金したい」

1. **Wallet** に入る → keystoreファイル + パスワードでアンロック
2. **Transfer WC** タブを選択 → 送金先アドレスと金額を入力 → **Send**

### 「自分のBlue Coinを発行したい」

1. **Wallet** に入る → アンロック
2. **Deploy** タブを選択
3. 入力：名前、シンボル、プール比率、チーム比率、初期WC、月次リリース量
4. **Deploy** をクリック → トークンとAMMプールが即座に稼働

### 「Blue Coinを取引したい」

1. **Wallet** に入る → アンロック
2. **Swap** タブを選択
3. トークンを選び、方向（WC→Blue or Blue→WC）を選び、金額を入力 → **Swap**
...
ノードは自動的にウォレットを生成し、テストネットに接続し、24時間の候補期間を開始します。検証後、ブロックごとに（15秒ごと）50 WCを獲得できます。

### 「チェーンのデータを見たいだけ」

[http://8.217.52.231](http://8.217.52.231) にアクセス — ブロック、トランザクション、バリデータ、Blue Coin、任意のアドレスを閲覧できます。
